...
bool ub_TPC_CardData_v6::compare(ub_TPC_CardData_v6 const& card_data,bool do_rethrow ) const
{
    try {
...
bool ub_TPC_CardData_v6::operator==(ub_TPC_CardData_v6 const& card_data) const {
    return compare(card_data,false);
}

uint32_t const& ub_TPC_CardData_v6::getCardTrigFrameAndSampleWord() const noexcept {
    return header().trig_frame_and_sample;
}

uint32_t ub_TPC_CardData_v6::getID() const noexcept {
    return header().getID();
}
uint32_t ub_TPC_CardData_v6::getModule() const noexcept {
    return header().getModule();
}
uint32_t ub_TPC_CardData_v6::getEvent() const noexcept {
    return header().getEvent();
}
uint32_t ub_TPC_CardData_v6::getFrame() const noexcept {
    return header().getFrame();
}
uint32_t ub_TPC_CardData_v6::getChecksum() const noexcept {
    return header().getChecksum();
}
uint32_t ub_TPC_CardData_v6::getWordCount() const noexcept {
    return header().getWordCount();
}
uint32_t ub_TPC_CardData_v6::getTrigFrameMod16() const noexcept {
    return header().getTrigFrameMod16();
}
uint32_t ub_TPC_CardData_v6::getTrigSample() const noexcept {
