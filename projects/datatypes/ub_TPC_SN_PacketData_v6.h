...
static_assert (sizeof(ub_TPC_SN_PacketHeader) == 2, "ub_TPC_SN_WidowHeader structure size is not correct.");
...
public:
    ...
    void decompress_into(std::vector<T>& uncompressed) const throw(datatypes_exception)
    {
      ...
      for(it = raw.begin(); it!= raw.end(); it++) {
        ...
        // Check top nibble for uncompressed word:
