...
namespace datatypes {
  ...
  void ub_ChannelDataCreatorHelperClass<ub_PMT_ChannelData_v6>::populateChannelDataVector(std::vector<ub_PMT_ChannelData_v6> & channelDataVector)
  {
      ...
      if(curr_rawData.size() < 2) {
        // Tagg May 2017
        // This condition happens when PMT card 4 is reading out in supernova stream. A single word of data is in the buffer: a single 0xC000 word
        // It probably should be empty or have empty channel headers, but this is distintive enough we can safely ingnore things.
        return;
      }
